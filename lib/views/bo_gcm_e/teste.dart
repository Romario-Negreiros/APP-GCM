...
class _BoGcmEViewState extends State<BoGcmEView> {
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 24),
                // --- SEÇÃO DE FOTOS DO CROQUI / AVARIAS ---
                const Text(
                  'CROQUI / AVARIAS',
                  style: TextStyle(fontWeight: FontWeight.bold),
                ),
  ...
  // --- WIDGET DE SLOT PARA FOTO ---
  Widget _buildPhotoSlot(ParteEnvolvida parte, int photoIndex) {
    final photo = parte.fotosCroqui[photoIndex];

    return GestureDetector(
      onTap: () {
        // Aqui entraria a lógica para abrir a câmera ou galeria
        // Exemplo futuro:
        // final pickedFile = await ImagePicker().pickImage(source: ImageSource.camera);
        // if (pickedFile != null) { ... }
        print(
          'Adicionar foto no slot $photoIndex para ${parte.nomeController.text}',
        );
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Funcionalidade de foto em breve (Slot ${photoIndex + 1})',
            ),
          ),
        );
      },
